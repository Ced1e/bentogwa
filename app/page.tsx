  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (token) {
        ...
        // Clean up the URL bar so the token disappears after loading the view
...
function ForgotPasswordView({ setView }: { setView: (v: any) => void }) {
  ...
  const [devLink, setDevLink] = useState(""); // For testing without an email server
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.devLink) setDevLink(data.devLink); // Display the link for testing
      } else {
