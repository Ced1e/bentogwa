      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-4 grid grid-cols-2 lg:grid-cols-1 gap-4 lg:gap-6">
            ...
            <div onClick={() => setView('signup')} className="col-span-2 lg:col-span-1 relative overflow-hidden bg-slate-900 rounded-[20px] p-6 shadow-md hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group border border-slate-800">
               {/* Clean, minimal background glow */}
               <div className="absolute top-0 right-0 w-40 h-40 bg-indigo-500 rounded-full blur-[80px] opacity-10 pointer-events-none group-hover:opacity-20 transition-opacity duration-500"></div>
               
               <div className="relative z-10 flex flex-col h-full justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <h3 className="text-lg font-bold text-white tracking-tight">Unlock BentoGWA Pro</h3>
                    <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-300 text-[9px] font-bold uppercase tracking-wider rounded border border-indigo-500/30">100% Free</span>
                  </div>
                  <p className="text-[13px] font-medium text-slate-400 mb-4 leading-relaxed">
                    Securely save your academic data to the cloud. Gain access to advanced analytics, PDF transcript exports, and predictive grade forecasting.
                  </p>
                  <p className="text-[10px] text-slate-500 font-medium mb-6 italic">
                    Built by a student, for students. No ads, no paywalls, no catch.
                  </p>
                </div>
                <div className="inline-flex px-5 py-2.5 bg-indigo-600 group-hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-all active:scale-95 w-fit items-center gap-2 shadow-sm">
...
const exportToPDF = async () => {
    ...
    try {
      ...
      let currentY = 0; // Tracks vertical position on the current page

      // Helper function: strictly typed with 'elementId: string'
      ...
      // 1. Snapshot the Header & Profile
      await addElementToPDF("pdf-header-section");

      // 2. Snapshot each Semester block individually
      const selectedSems = analytics.processedSemesters
        .filter(sem => selectedExportSems.includes(sem.id))
        .sort((a,b) => a.id - b.id);

      for (const sem of selectedSems) {
        await addElementToPDF(`pdf-sem-${sem.id}`);
      }

      // Handle the edge case where no semesters are selected
      if (selectedSems.length === 0) {
         await addElementToPDF("pdf-empty-state");
      }
    ...
    <>
      ...
      */}
      ...
      >
        ...
        <div id="pdf-header-section" style={{ padding: "40px 50px 20px 50px", background: "#ffffff" }}>
        ...
        {/* BLOCKS 2+: INDIVIDUAL SEMESTER SECTIONS */}
        {analytics.processedSemesters
          ...
          .map(sem => {
            ...
            return (
            <div id={`pdf-sem-${sem.id}`} key={sem.id} style={{ padding: "20px 50px", background: "#ffffff" }}>
        ...
        {selectedExportSems.length === 0 && (
          <div id="pdf-empty-state" style={{ padding: "20px 50px", background: "#ffffff", textAlign: "center" }}>
            <p style={{ color: "#64748b", fontSize: "12px", fontStyle: "italic" }}>No semesters selected for export.</p>
          </div>
        )}
      </div>
    </>
